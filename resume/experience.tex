\vspace{-2.0em}
\cvsection{Working Experience}
\begin{cventries}
    \cventry
    {Senior Blockchain Engineer - Protocol}
    {ChainSafe Systems - Gossamer(Polkadot)}
    {Global}
    {Feb. 2025 - Now}
    {
      \begin{cvitems}
        \item {Gossamer is a Go implementation of Polkadot node.}
        \item {It implements Polkadot Host functions such as consensus, networking, state machine, etc.}
        \item {Responsible for development of subsystems for the parachain block candidate lifecycle such as Bitfield Signing subsystem, Bitfield Distribution subsystem, etc. which aligning and compatible with Polkadot SDK implementation.}
      \end{cvitems}
    }

    \cventry
    {Senior Blockchain Engineer - Web3 Bridging}
    {ChainSafe Systems - Sygma(Previously ChainBridge)}
    {Global}
    {Feb. 2022 - Mar. 2025}
    {
      \begin{cvitems}
        \item {Sygma is a Crosschain bridging protocol connecting between EVM and Polkadot.}
        \item {Lead the development and maintaining the Sygma Crosschain Bridge on Polkadot side inlcuding Substrate pallets, parachain runtime development, integration process between sygma relayer and substrate chains(e.g: Phala), production release, mainnet and testnet release.}
        \item {Responsible for design and develop the integration between Golang module and Substarte Rust module, AKA Polkadot Parachain runtime integration.}
        \item {Responsible for Sygma Mainnet Polkadot execution and configuration.}
        \item {Onboarding other Polkadot-Substrate chains onto Sygma bridge.}
      \end{cvitems}
    }

    % \cventry
    % {Team Lead - Engineer Management}
    % {ChainSafe Systems - Ethermint and Cosmos Module for Chainlink(Cosmos)}
    % {Global}
    % {Aug. 2021 - Nov, 2022}
    % {
    %   \begin{cvitems}
    %     \item {Lead the development work on Cosmos-Chainlink module(Detail is in Project section).}
    %     \item {Designed the architecture and facilitate knowledge transferrring across the organization.}
    %     \item {Progress reporting to C level stakeholders.}
    %     \item {Team management. Facilitate 1 on 1 meetings with engineers, resource }
    %     % \item {Lead the development work on GTS(Gasless Transaction as Service)(Detail is in Project section).}
    %     % \item {Developer \& Researcher of the ChainSafe ChainBridge(Detail is in Project section).}
    %   \end{cvitems}
    % }
    
    \cventry
    {Blockchain Developer - Protocol}
    {ChainSafe Systems - Ethermint and Cosmos Module for Chainlink(Cosmos)}
    {Global}
    {Jan. 2021 - Aug. 2021}
    {
      \begin{cvitems}
        \item {Tech: Go, NodeJS, Cosmos, IBC, Ethermint, Tendermint.}
        \item {Revamp the consensus layer to be compatible with Tendermint core.}
        \item {Revamp the application layer to be compatible with EVM.}
        \item {Revamp Ethermint RPC layer.}
        % \item {Designed and developed GTS(Gasless Transaction as Service)(Detail is in Project section).}
      \end{cvitems}
    }
    
    \cventry
    {Blockchain Payment System Developer}
    {BitSpawn}
    {Toronto. ON. Canada}
    {Jul. 2019 - Dec. 2021}
    {
      \begin{cvitems}
        \item {Tech: Go, NodeJS, BitCoin, Solana, Ethereum, Solidity, Etherscan-api, BlockCypher-api, ERC-20 Standard, PostgreSQL, Redis, Bloom Filter, HD wallet, KMS, BitGo, Paypal, Stripe}
        \item {Designed and implemented Deposit Manager(Detail is in Project section).}
      \end{cvitems}
    }

    \cventry
    {Blockchain Protocol Developer}
    {OneLedger}
    {Toronto. ON. Canada}
    {Apr. 2019 - Jan. 2021}
    {
      \begin{cvitems}
        \item {Tech: Go, Typescript, NodeJS, Tendermint, PBFT, LevelDB, HD key Derivation, PostgreSQL, SQS, Linux}
        % \item {Developed Blockchain RPC endpoint layer.}
        \item {Developed Blockchain Transaction Verification layer.}
        \item {Developed Blockchain Transaction Check and Distribution layer.}
        \item {Developed Blockchain Fullnode and Validator node block scope layer.}
        \item {Developed Blockchain Distributed Database layer.}
        \item {Designed and developed Oneledger HD Wallet that supports BTC, ETH and OLT(Oneledger) key derivation.}
        \item {Designed and developed Oneledger Blockchain SDK.}
        \item {Designed and developed Oneledger Blockchain Explorer.}
      \end{cvitems}
    }

  \cventry
    {Blockchain Application Developer}
    {SkyQuark}
    {Toronto. ON. Canada}
    {Apr. 2018 - Apr. 2019}
    {
    %   \begin{cvitems}
    %     \item {Tech: Ethereum, geth, parity, Solidity, Truffle, Mist, web3, IPFS, EOS, Docker, koa framework}
    %     \item {Designed and developed Skyquark Crowdsourcing system back-end using NodeJS, MongoDB, LevelDB, web3JS, Parity}
    %     \item {Designed and developed Skyquark Aristotle Chain using Parity.}
    %     \item {Designed and developing the Aristotle token system within Edusphere eco-system.}
    %     \item {Developed Edusphere Intellectual Property protection dApp(smart contracts) on Aristotle Chain.}
    %     \item {Developed Edusphere certificate verification dApp(smart contracts) on Aristotle Chain.}
    %     \item {Designed and developed Skyquark ICO smart contract(ERC-20)}
    %     \item {Provided 'Blockchain for Junior' online lectures as instructor at www.edusphere.io}
    %     \item {Developed an EOS based PoC of Aristotle Chain prototype.}
    %   \end{cvitems}
    }
	

  \cventry
    {Security platform Automation Developer Intern}
    {BlackBerry}
    {Waterloo. ON. Canada}
    {May. 2017 - Sept. 2017}
    {
    %   \begin{cvitems}
    %     \item {Tech: Android, Jenkins, Shell Script, UiAutomatorViewer, JUnit4, Linux, Git, JIRA, SSH, Security key exchange}
    %     \item {Developed security key based unit test cases including S/MIME, PGP and large size Sign and Encrypt key.}
    %     \item {Developed test sanity to automatically build testing job for different BlackBerry app components.}
    %     \item {Set up and configure Jenkins server and nodes to organize BlackBerry security message jobs.}
    %     \item {Designed and developed Monkey command tool App for automation and pressure test on BlackBerry apps.}
    %   \end{cvitems}
    }
  
%   \cventry
%     {Front-end Developer}
%     {University of Waterloo - Renison University College}
%     {Waterloo, ON}
%     {Nov. 2015 - Apr. 2016}
%     {
%       \begin{cvitems}
%         \item {Tech: Java, JavaScript, jQuery, HTML5, CSS3, Augmented Reality}
%         \item {Designed and developed an interactive web application to demonstrate the history of the relationship between Renison University College and Anglican Religion.}
%         \item {It contains three parts: a JavaScript puzzle game, an augmented Reality photo and an information tree developed by jQuery.}
%         \item {Awarded Outstanding Project by the Anglican Religion Group.}
%       \end{cvitems}
%     }
    
%   \cventry
%     {Full Stack Developer}
%     {TCSL}
%     {Tianjin, China}
%     {Feb. 2015 - Aug. 2015}
%     {
%       \begin{cvitems}
%         \item {Tech: JavaEE, JavaScript, jQuery, Struts2, Spring3, Hibernate4, Linux, Maven, Nexus, Tomcat, PostgreSQL, SVN}
%         \item {Participated in design and development the architecture and backend framework.}
%         \item {Built a local maven component server for the entire developing group.}
%         \item {Developed business components of data persistence using Hibernate.}
%         \item {Developed business transactions control unit using Spring.}
%         \item {Participated in the development of user interfaces.}
%       \end{cvitems}
%     }
    
  \cventry
    {Back-end Developer Intern}
    {Oracle}
    {Tianjin, China}
    {Sept. 2014 - Feb. 2015}
    {
    %   \begin{cvitems}
    %     \item {Tech: JavaEE, AJAX, jQuery, DBCP, MySQL, Tomcat, Apache Component lib, MVC, Linux, SSH, Git}
    %     \item {Developed a social network system based on JavaEE(SSH framework) and MySQL.}
    %     \item {Designed the database and implemented with PowerDesigner.}
    %     \item {Designed the architecture and using MVC for implementation.}
    %     \item {Awarded Best Intern Project by Oracle.}
    %   \end{cvitems} 
    }
\end{cventries}
